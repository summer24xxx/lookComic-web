...
export default {
  ...
  created() {
    ...
    $(window).on("scroll.a", () => {
      ...
      if (this.position.top === undefined) {
        ...
        if (scrollTop > this.position.scrollTop) {
          // 向下滚动
          ...
          if (scrollTop > this.position.top + (4 * height) / 5) {
            // 下一页
            this.nextPage();
          }
        } else {
          ...
          if (scrollTop < this.position.top + height / 5) {
            // 上一页
            this.prevPage();
          }
        }
  ...
  methods: {
    ...
    appendPages() {
      let nowLoaded = this.imgList.length
        ? this.currentPage + 1
        : this.currentPage;
      if (
        !this.imgList.length ||
        nowLoaded === this.imgList[this.imgList.length - 1] + 1
      ) {
